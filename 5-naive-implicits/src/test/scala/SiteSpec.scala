...
class SiteSpec extends Specification {
  ...
  "routes can be invoked directly" in {
    ...
    Calculator.square(Tuple1(5))                               mustEqual Response("5 ^ 2 = 25")
    Calculator.stringMultiply(("abc", 2))                      mustEqual Response("abc * 2 = abcabc")
    Calculator.stringAppend(Tuple1(List("abc", "def", "ghi"))) mustEqual Response("append(List(abc, def, ghi)) = abcdefghi")
  }
  ...
  "routes produce the correct urls" in {
    ...
    Calculator.square.url(Tuple1(5))                               mustEqual "/square/5"
    Calculator.stringMultiply.url(("abc", 2))                      mustEqual "/multiply/abc/by/2"
    Calculator.stringAppend.url(Tuple1(List("abc", "def", "ghi"))) mustEqual "/append/abc/def/ghi"
  }
